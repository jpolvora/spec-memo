...
**Error logs viewer** (`?tab=error-logs`): `GET /api/error-logs?limit=&offset=&level=&subsystem=` returns `{ items, total, truncated }` (`limit` default 50, max 200; `level` one of `ERROR|WARN|FATAL`; invalid query 400; unauthenticated 401 with no log bodies). Items carry `id`, `timestamp`, `level`, `subsystem`, `error` (truncated to 300 chars) plus optional `endpoint`/`tool`/`projectId` — never the raw stack. `GET /api/error-logs/{id}` returns one parsed block with truncated stack/context (unknown id 404). The parser splits vault `error.logs` on the `====` block delimiters, scans at most the last 2 MiB newest-first (`truncated: true` when larger), and a missing file yields `200` with an empty list. Payloads pass `sanitizeToolOutput`; the row-click detail pane fills `<pre>` via `textContent` (escaped, never `innerHTML` of log text). Fetch 401/500 shows an inline page error; handler exceptions log to `error.logs` under subsystem `status-server` with endpoint `/api/error-logs`.

**AI assistant config** (`?tab=ai-config`): `GET /api/config/ai` returns `{ enabled, provider, model, apiKeyEnv, hasApiKey, available }` with no secret values (`provider` reads `noop` when disabled, even if disk keeps `cursor-sdk`; `hasApiKey` is boolean only). `PUT /api/config/ai` accepts only `enabled` (boolean), `provider` (`noop`|`cursor-sdk`), `model` (non-empty, max 64 chars); extra keys are ignored while `apiKey`/`token` fields yield 400 without writing; `opencode`/`freellmapi`/other providers yield 400 (only `noop` and `cursor-sdk` are implemented). `noop` persists `ai.enabled: false`; `cursor-sdk` persists `enabled: true`, `provider: "cursor-sdk"`, `model` (omitted model preserves the stored value; a fresh vault defaults to `composer-2.5`). A body with neither `provider` nor `enabled` yields 400 without writing. The write merges only the `ai` object into vault `config.json` under `withVaultLock` (preserving `ttl`, `vaultGit`, `embeddings`, `bootstrap`, `wiki`, `ports`, and unknown future `ai` fields such as `opsLogEnabled`) via an atomic tmp+fsync+rename write, and later reads reflect it without a restart. The UI shows a provider select (`noop` = Disabled/Noop, `cursor-sdk` enabled, `opencode`/`freellmapi` disabled with Coming soon hint), a model input (default `composer-2.5`), a Save button calling `PUT`, and a note that the key lives in `apiKeyEnv` only — the key is never displayed, sent, or stored.
