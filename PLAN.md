...
| 2026-09-06 | status-vaults-tab-ui | Status Vaults tab modal forms + POST /api/vaults/sync; v0.27.1 | npm test 615 pass |
| 2026-09-06 | status-vaults-tab-ui-polish | Status Vaults tab Actions column visual polish, side-by-side button grid, semantic accents, responsive layout; v0.27.2 | npm test 615 pass |
...
| 2026-09-08 | vault-merge-ui-and-acyclicity-polish | Status monitor merge modal target ID & coupled controls, vault-manager merge cycle pre-validation & alias clearing; v0.28.1; npm test 654 pass |
| 2026-09-10 | us-55-sync-partial-failure | Vault-git `pull --rebase --autostash` dirty-tree hardening + `applyChangeset` AC6 offender skip-and-log (`skipped`/`conflictDetails`, capture stays strict); npm test 659 pass |
| 2026-09-12 | us-55-followup-evidence | Dual-sync sequential hybrid→vaultGit (ends autostash pile-up race), vault-git autostash abort+pop recovery, `applyChangeset` transient view-rebuild skip-and-log per record/project with retry, dual-sync dirty-state error surfacing + `all` flag preserved in fallback; v0.28.3; npm test 667 pass |
| 2026-09-12 | memo-shutdown-56 | `memo shutdown` (alias `stop`) graceful stop for orphaned serve processes: command-line discovery, SIGTERM-then-force, vaultRoot scoping, dry-run, canvas opt-in, JSON report; README/AGENTS/ws-memo docs; v0.28.3; npm test 683 pass |
...
| 2026-09-13 | mcp-io-guard | MCP I/O guard: closed-table prompt-intent scan, IO_GUARD inbound refuse, query drop-not-fail, outbound untrusted fences + SHA-256 ioGuard/ioChecksum, hybrid skip-and-log, ws-memo Agent I/O, bootstrap over-budget handoff claim fix; v0.33.0; npm test 809 pass |
