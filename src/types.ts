...
export interface RecordFrontmatter {
  ...
  links?: RecordLink[];
  /** AI retrieval aids: keyword phrases generated by the vault AI agent (AC15). Never replaces body. */
  aiSearchTerms?: string[];
  /** AI retrieval aid: one-paragraph summary for search, max 500 chars (AC15). */
  aiSummary?: string;
...
export interface VaultConfig {
  ...
  /**
   * Optional vault intelligence layer (spec 0056). Omit or set
   * `enabled: false` (default) to run fully offline. The API key is never
   * stored here — it is read from `process.env[ai.apiKeyEnv]`.
   */
  ai?: VaultAiConfig;
  ttl: {
    scratchDays: number;
...
export type AiRankDisposition = 'applied' | 'skipped';

/**
 * Dedicated vault `config.json` `ai` section (spec 0056, AC27).
 * Zod-validated in `src/ai/config.ts`; mirrored here as the canonical type.
 */
...
/**
 * Read-only AI status for doctor `--json` and `/api/status` (AC28–AC29).
 */
export interface VaultAiStatus {
  enabled: boolean;
  provider: 'cursor-sdk';
  available: boolean;
  queueDepth: number;
...
export interface SearchScoreExplain {
  ...
  /** Post-FTS AI rerank disposition (spec 0056, AC22). Non-budget diagnostic. */
...
export interface DoctorResult {
  ...
  /** Optional vault AI assistance status (spec 0056, AC28). Secrets stripped. */
  ai?: VaultAiStatus;
}
