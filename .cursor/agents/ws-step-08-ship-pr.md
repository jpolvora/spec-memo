---
name: ws-step-08-ship-pr
description: "Workflow step agent 08. Internal orchestrator dispatch only; do NOT invoke autonomously."
disable-model-invocation: true
---

<!-- @generated by workflow-skills compile_host_subagents.cjs. DO NOT EDIT DIRECTLY. hash:2ad49ca4a39749556607605b66ad2457c654e0fb61083fc262d5ecea2f883c64 -->

# Specialized Subagent — Step 08 (ship-pr)

> Compiled from canonical skill: `.agents/skills/ws-ship-pr/SKILL.md`
...
- **`providers.scm: "github"`** → dispatches [`ws-spec-provider-github`](../../.agents/skills/ws-spec-provider-github/SKILL.md) (`gh` CLI / GitHub REST API)
- **`providers.scm: "azure-devops"`** (or `"ado"`) → dispatches [`ws-spec-provider-azure-devops`](../../.agents/skills/ws-spec-provider-azure-devops/SKILL.md) (`az repos` / ADO REST API)
- **`providers.scm: "local"`** → dispatches [`ws-spec-provider-local`](../../.agents/skills/ws-spec-provider-local/SKILL.md)

Prepare board (mandatory): [PREPARE-CHECKLIST.md](../../.agents/skills/ws-ship-pr/PREPARE-CHECKLIST.md). Wait/converge timing: [GOAL-OVERRIDES.md](../../.agents/skills/ws-ship-pr/GOAL-OVERRIDES.md). Examples: [examples.md](../../.agents/skills/ws-ship-pr/examples.md).
...
See [`gates.md`](../../.agents/skills/ws-shared/runtime/gates.md) § Quality gate bypass. Ship/PREPARE row 5 nuances: [PREPARE-CHECKLIST.md](../../.agents/skills/ws-ship-pr/PREPARE-CHECKLIST.md) § 5. Safety floor (REFUTED) never bypassed.
...
5. **Create PR**: only when Step 2 is green and `shipAction: create-pr` (or standalone default). Resolve `providers.scm` per [`config-resolution.md`](../../.agents/skills/ws-shared/runtime/config-resolution.md) (`github` or `azure-devops` / `ado` only for create-pr; STOP if `local` or unresolved — do not invent a client). Load matching provider ([ws-spec-provider-github](../../.agents/skills/ws-spec-provider-github/SKILL.md) or [ws-spec-provider-azure-devops](../../.agents/skills/ws-spec-provider-azure-devops/SKILL.md)), `validate-auth` (STOP on failure), then `create-pr --head {shipHead} --base {baseBranch}` (reuse open PR for same head→base when present). Capture PR id and URL. When workflow state or spec frontmatter has tracker `id`, dispatch provider **`comment-issue`** (alias `close-loop`) with PR URL + one-paragraph summary (`dry-run` when parent is dry-run). Skip when `id` is null / `source: local`.
...
6. **Monitor reviews & converge**: skip if `stopBeforeFixPr` (orch Step 9 owns [ws-goal-fix-pr](../../.agents/skills/ws-goal-fix-pr/SKILL.md)). Otherwise, after pushing and creating PR, wait **30 seconds** (wait for code-review action / CI workflows to start on SCM infrastructure), then start [ws-goal-fix-pr](../../.agents/skills/ws-goal-fix-pr/SKILL.md) (default **300 seconds** heartbeat/settle loop, [GOAL-OVERRIDES.md](../../.agents/skills/ws-ship-pr/GOAL-OVERRIDES.md)), poll required checks via provider **`check-pr-status`** (classify diff-regression vs baseline vs infra-flake; one flake rerun; baseline does not block merge only when reproduced on default branch and recorded) and `list-threads` via the configured SCM provider, and dispatch `ws-goal-fix-pr` until `activeThreads == 0` or `max`. Never merge while threads remain, checks are red, or on escalate-stop. Prepare the handoff prompt/state for `ws-goal-fix-pr` even when stopping early so Step 9 can resume cleanly.
...
- Prepare board: [PREPARE-CHECKLIST.md](../../.agents/skills/ws-ship-pr/PREPARE-CHECKLIST.md) · Verify helper: `bash {skillsRoot}/ws-ship-pr/scripts/verify.sh`
- SCM Providers (configured via `config.json` `providers.scm`): [ws-spec-provider-github](../../.agents/skills/ws-spec-provider-github/SKILL.md) · [ws-spec-provider-azure-devops](../../.agents/skills/ws-spec-provider-azure-devops/SKILL.md) · [ws-spec-provider-local](../../.agents/skills/ws-spec-provider-local/SKILL.md)
- Security: [ws-secrets-leak-review](../../.agents/skills/ws-secrets-leak-review/SKILL.md)
- Review: [ws-code-review](../../.agents/skills/ws-code-review/SKILL.md) · Convergence: [ws-goal-fix-pr](../../.agents/skills/ws-goal-fix-pr/SKILL.md) · Fixer: [ws-fix-pr](../../.agents/skills/ws-fix-pr/SKILL.md)
- Base detection: `bash {skillsRoot}/ws-ship-pr/scripts/detect-base-branch.sh` · Artifacts: [ARTIFACTS.md](../../.agents/skills/ws-spec-to-pr/ARTIFACTS.md)
