...
Package version: **0.37.4** (`develop`). Status marks: `[ ]` planned · `[~]` in progress · `[x]` shipped (proof in [`PLAN.md`](PLAN.md)).
...
- [x] **Compiled views.** Regenerated, never hand-edited: `INDEX.md`, `TRAPS.md`, `DECISIONS.md`, `PROMPTS.md`, `SESSIONS.md`. Rebuild from sources (no merge of compiled files). Writes are atomic (sibling temp file plus rename) with bounded retries on transient Windows locks (`EPERM`/`EBUSY`/`UNKNOWN`/`EACCES`); persistent view failures degrade to skip-and-log in sync apply.
- [x] **Record source values.** Known trio `agent` / `human` / `imported` (normalized lowercase) plus any non-empty origin slug (e.g. workflow skill names like `ws-configure-project`), preserved verbatim; empty values still fail validation.
...
- [x] **Per-project vault wiki.** On-demand `WIKI.md` at `projects/{projectId}/WIKI.md` (never product README), status **Wiki** tab with left topic menu + Index/section progressive disclosure, CLI `memo wiki` / `memo wiki --regenerate`. Index-first Topic catalog template. Optional AI polish from a fresh vault snapshot when `ai.enabled` (fail-open; same agent choke point as refine). Not an MCP tool.
- [x] **Vault project manager (alias + merge).** `config.json` `projectAliases` redirects duplicate `projectId`s to a canonical folder; status **Vaults** tab and `memo vault` CLI for list/create/alias/merge/update/delete. Optional `copyRecords` on merge. Not an MCP tool.
...
- [x] **Hybrid Bidirectional Sync & Debounced Push.** Low-latency local cache with automatic remote delta pulls on `bootstrap` (fail open) and debounced push scheduling after mutating operations (`upsert`, `append`, `forget`, `gc`). Unreachable-daemon outcomes (`fetch failed`, timeouts, refused) log at `WARN` with structured `lastError` in hybrid-state; data-risk outcomes (journal restore failure, unrecovered autostash, torn index) keep `ERROR`.
- [x] **Batched vault-git + dual-mode hybrid dispatch.** `vaultGit.atomic` (default `false`) defers git commit/push to `memo sync`, `session_end`, and graceful serve shutdown; `atomic: true` restores per-mutation commit+push. Hybrid HTTP runs first, then vault-git, sequentially in one run when both are enabled (dual-mode), fail-open, with `error.logs` (`subsystem: vault-git`). Vault-git pull uses `git pull --rebase --autostash` so daemon re-dirtied trees no longer abort the pull; pull failures abort the rebase and pop leftover autostash instead of leaking it; transient view-rebuild failures skip-and-log per record/project with retry; sync-apply skips/logs per-record AC6 fully-ignored `pathPatterns` offenders (`skipped` + `conflictDetails`, direct capture stays strict).
...
- [x] **Operational Telemetry & Rolling Usage Logging.** Configurable switch `enableTelemetry` in `config.json` (default `true`); append-only structured JSONL streaming under `$SPEC_MEMO_ROOT/telemetry/`; rolling daily part rotation (`telemetry-YYYY-MM-DD.part-N.jsonl`) by file size limit (`maxFileSizeMb`); `memo status` reports the latest rolling part file (not `usage.jsonl`); bounded CLI/status/doctor summaries split expected CLI/auth failures from product faults; non-blocking fail-safe queue with zero-crash error isolation and secret redaction.
- [x] **Doctor ignore + gitignore classification.** `.spec-memo-ignore` still suppresses residue; gitignored `.agents/plans/` files are classified and do not fail `memo doctor`. Live vault-git porcelain is reported beside persisted `.sync/vault-git-state.json`.
...
- [x] **Cross-agent session handoff batons.** Owner/branch-isolated tactical batons written on `session_end` (`handoff` payload / `--handoff-steps`), delivered once via `bootstrap` or `session_start`, optional `--shared` team batons, `memo session handoff` CLI, status monitor Active Handoffs panel, and `cancel_handoff` action.
...
- [x] **Graceful shutdown for orphaned serve processes (`memo shutdown`).** `memo shutdown` (alias `stop`) discovers running `memo serve` stdio/SSE instances by command line, sends SIGTERM so each server's own handlers flush (vault-git + hybrid, fail-open), waits `--timeout-ms` (else `SPEC_MEMO_SYNC_TIMEOUT_MS`, else 8000 ms), then force-terminates survivors with PID-reuse revalidation. `--vaultRoot` scoping, `--dry-run` preview, `--include-canvas` opt-in (canvas excluded by default), `--force`, `--json` report, redacted `--auth-token` in output. Never kills self, PID 0/1, unreadable entries, or non-serve processes.
- [x] **Vault AI assistance (optional, off by default).** Pluggable `VaultAiAgent` (`isAvailable`/`refineForSearch`/`rankCandidates`) with `NoopVaultAiAgent` default and `CursorSdkVaultAiAgent` (`@cursor/sdk` `Agent.prompt` one-shot, cloud no-repo runtime, key via `CURSOR_API_KEY` env, never in config). Background refine-on-write for `trap`/`decision`/`spec`/`plan` writes retrieval aids only (`aiSearchTerms`/`aiSummary`/`aiRefineHash`, body untouched, FTS rebuilt under lock); post-FTS top-K rerank on `search`/`bootstrap` with fail-open `explain.aiRank`; `get` never calls the agent. `memo doctor --json` and `/api/status` expose read-only `ai` (`enabled`, `provider`, `available`, `queueDepth`, `lastError`); activity emits `ai.refine.*`/`ai.rank.*`. Still 11 MCP tools; Cursor not required.
- [x] **AI Ops journal & connectivity test.** Durable redacted analysis log under vault `ai-ops/` (`operation` refine/rank/test) with status-monitor **AI Ops** tab, `GET /api/ai-ops` list/detail filters, and a **Test AI** button that probes the configured provider/model via `POST /api/ai-ops/test` and force-logs the `test`/hello settlement even when AI is disabled.
- [x] **Vault stability audit (0067).** Read-only doctor/diagnosis (no scaffolding of missing vaults or malformed configs); FTS identity-set compare with named missing/unexpected ids; dual-sync per-channel ok/code/phase/duration/dirty-transition/cursor-impact truth; sequential dual dispatch wording everywhere; backup-gated vault merge (`--confirm`, fresh `--backup-id`, `--manifest-reviewed`, quarantine refusal) with hybrid cursor/dirty migration on rename/merge; bounded telemetry summaries (failure rate, per-project health, six observable counters) in doctor/status; generic spec-lifecycle drift scan.
