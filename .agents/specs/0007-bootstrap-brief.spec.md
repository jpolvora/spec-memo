...
- AC3: The total brief payload size is strictly capped at the token budget (default 8,192 bytes UTF-8 from vault `config.bootstrap.maxBytes`; per-call `maxBytes` overrides).
...
|------------|----------------|-----------|-----------|
| Default byte budget | 8,192 bytes (8 KB) via `config.json` `bootstrap.maxBytes`; per-call `maxBytes` overrides | PRD constraint §6; prevents LLM context window pollution; operators may raise the cap | y |
