...
4. **Ship in two phases (OpenViking-shaped, spec-memo-sized):**
   - **Phase A (this slice):** `originRelPath` frontmatter + FTS index + `--include` globs + `--dry-run` manifest + read-only `resources/list`+`resources/read` + `get`/`search`/`bootstrap` `path` + synthetic directory overviews + `doctor` pollution parity. No new tools.
