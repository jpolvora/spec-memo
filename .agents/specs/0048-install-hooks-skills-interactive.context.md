...
This slice changes **how operators pick destinations** for two existing installers. It does not change vault MCP tools other than adding fail-closed `confirm` / `hosts` / `scope` on `install_skills`. Hook **event mappings** for Antigravity, OpenCode, Cursor, and Claude stay as in `0041-agent-hooks-install.spec.md`. Skill file contents stay the packaged `ws-memo` / `ws-session-tracking` trees.
