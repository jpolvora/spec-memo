...
- AC3: The CLI command `memo install-hooks` accepts options: `--host <host>`, `--global`, `--dry-run`, `--apply`, `--force`, and `--json`.
...
- AC8: For Cursor, `install-hooks` generates an active rule `.cursor/rules/spec-memo.mdc` configured with `alwaysApply: true` instructing Cursor Agent to invoke `bootstrap` on intake and record session summaries into `spec-memo`.
- AC9: Cursor rule files include file-pattern frontmatter ensuring they apply across all source files in the project.

### Non-Blocking Fail-Open Execution Contract

- AC10: All shell and script hooks generated by `install-hooks` invoke `memo` with strict execution timeouts (maximum 1500ms) and redirect stdout/stderr to avoid hanging agent startup.
- AC11: If `memo` executable is not found or fails with non-zero exit code, generated hooks exit 0 silently (fail-open), ensuring agent workflows never halt due to memory tooling.

### Installation Safety & CLI UX

- AC12: Running `memo install-hooks` without `--apply` performs a dry-run preview, displaying the exact target paths and content diffs to be written without modifying the filesystem.
- AC13: Passing `--apply` creates necessary parent directories and writes configuration files atomically, creating timestamped `.bak` copies if target files already exist and differ.
- AC14: Passing `--json` emits structured machine-readable results (`{ host, path, status: 'installed' | 'unchanged' | 'preview', diff }`).

### Optional Hook Decoupling & Doctor Diagnostics

- AC15: `spec-memo` functions with 100% operational feature completeness in Skill-Only Mode using only the autoloaded `ws-memo` skill, without requiring `memo install-hooks`.
- AC16: When lifecycle hooks are not installed, `memo doctor` reports `Agent Hooks: Not installed (Skill-only mode active via ws-memo)` as an informational notice rather than an error or warning.
